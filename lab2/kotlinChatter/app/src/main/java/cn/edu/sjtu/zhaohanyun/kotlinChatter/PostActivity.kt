...
//    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val forCameraResult = registerForActivityResult(ActivityResultContracts.TakePicture()) { success ->
            if (success) {
                doCrop(cropIntent)
            } else {
        ...
        val forVideoResult = registerForActivityResult(ActivityResultContracts.CaptureVideo()) {
            viewState.videoIcon = android.R.drawable.presence_video_busy
            view.videoButton.setImageResource(viewState.videoIcon)
        }
        view.videoButton.setOnClickListener {
            viewState.videoUri=mediaStoreAlloc("video/mp4")
            forVideoResult.launch(viewState.videoUri)
        }
